...
void glopTexImage2D(GLContext *c,GLParam *p)
{
  ...
  if (!(target == GL_TEXTURE_2D && level == 0 && components == 3 && 
    ...
    gl_fatal_error((char *)"glTexImage2D: combinaison of parameters not handled");
  }
  
  printf("debug:texture.cpp: start glopTexImage2D\n");
  ...
  printf("debug:texture.cpp: image resize complete\n");
...
void glopTexEnv(GLContext *c,GLParam *p)
{
  ...
  error:
    gl_fatal_error((char *)"glTexParameter: unsupported option");
  }
...
void glopPixelStore(GLContext *c,GLParam *p)
{
  ...
  if (pname != GL_UNPACK_ALIGNMENT ||
      param != 1) {
    gl_fatal_error((char *)"glPixelStore: unsupported option");
  }
}
