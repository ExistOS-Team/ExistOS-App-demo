...
ZBuffer *ZB_open(int xsize, int ysize, int mode,
    ...
    printf("info: zbuffer.cpp: start ZB_open\n");

    zb = (ZBuffer *) gl_malloc(sizeof(ZBuffer));
    if (zb == NULL) {
        printf("error: zbuffer.cpp: cannot alloc space for zb\n");
        return NULL;
    }
    
    printf("debug: zbuffer.cpp: create zb space at %x\n", zb);
...
#endif
        case ZB_MODE_5R6G5B:
            printf("debug: zbuffer.cpp: ZB_open: ZB_MODE_RGBA/ZB_MODE_RGB24/ZB_MODE_5R6G5B");
            zb->nb_colors = 0;
            break;
        default:
            goto error;
    ...
    if (zb->zbuf == NULL) {
        printf("error: zbuffer.cpp: cannot alloc space for zb of zbuffer!");
    ...
    if (frame_buffer == NULL) {
        ...
        if (zb->pbuf == NULL) {
            printf("error: zbuffer.cpp: cannot alloc space for pixelbuffer(%d) of zbuffer!\n", zb->ysize * zb->linesize);
            gl_free(zb->zbuf);
            goto error;
        }
