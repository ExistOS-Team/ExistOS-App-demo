...
static uint8_t framebuffer[256 * 128];
//extern uint8_t *vram;
...
static inline void I_CopyFrameBufferRGB565(void)
{
	for(int y = 0; y < SCREENHEIGHT; y++)
	{
		for(int x = 0; x < SCREENWIDTH; x++)
		{
            ...
            framebuffer[offset] = rgb565_palette[I_VideoBuffer[offset]];
...
static inline void I_ScaleFBNearestNeighbourRGB565(void)
{
	...
	for (int y = 0; y < DOOMGENERIC_RESY; y++)
	{
		...
		for (int x = 0; x < DOOMGENERIC_RESX; x++)
		{
            ...
            framebuffer[i_x_colors] = rgb332_palette[I_VideoBuffer[y2_x2_colors]] ; //rgb565_palette[I_VideoBuffer[y2_x2_colors]];
...
void I_FinishUpdate (void)
{
    ...
    ll_disp_put_area(framebuffer,0,0,255,126);
	DG_DrawFrame();
}
...
void I_SetPalette (byte* palette)
{
	for (int i=0; i < 256 ; i++)
	{
        ...
        rgb332_palette[i] = rgb565_palette[i];
		palette += 3;
	}
